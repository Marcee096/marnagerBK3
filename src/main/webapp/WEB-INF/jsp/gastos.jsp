...
<div class="sidebar">
    ...
    <a href="${pageContext.request.contextPath}/gastos" class="active"><i class="fas fa-arrow-down"></i> Gastos</a>
...
<div class="main-content">
    <div class="container-fluid">
        <div class="page-header">
            ...
            <div>
                <form id="monthFilterForm" action="${pageContext.request.contextPath}/gastos" method="get" class="d-flex align-items-center">
        ...
        <c:if test="${not empty status}">
            <div class="alert alert-${status == 'created' || status == 'updated' ? 'success' : (status == 'deleted' ? 'info' : 'danger')} alert-dismissible fade show" role="alert">
                ...
                <strong>
                    <c:choose>
                        <c:when test="${status == 'created'}">¡Éxito! El gasto ha sido guardado.</c:when>
                        <c:when test="${status == 'updated'}">¡Éxito! El gasto ha sido actualizado.</c:when>
                        <c:when test="${status == 'deleted'}">¡Éxito! El gasto ha sido eliminado.</c:when>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <div class="empty-state">
                                    ...
                                    <p>No hay datos de gastos para mostrar en el gráfico para el mes seleccionado.</p>
                                </div>
                            </c:otherwise>
                        </c:choose>
                    </div>
                    ...
                    <div class="card-body-custom">
                        <form action="${pageContext.request.contextPath}/gastos" method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="subcategoria" name="subcategoria" placeholder="Ej: Supermercado, Factura de luz, etc." required>
                            </div>
                            <div class="row">
                            ...
                            <button type="submit" class="btn btn-primary w-100"><i class="fas fa-plus"></i> Añadir Gasto</button>
                        </form>
                    </div>
                </div>
            </div>
                                ...
                                <tbody>
                                    <c:forEach var="gasto" items="${gastos}">
                                        <tr>
                                            ...
                                            <td><span class="badge bg-danger-subtle text-danger-emphasis rounded-pill"><c:out value="${gasto.categoria}" /></span></td>
                                            <td><c:out value="${gasto.subcategoria}" /></td>
                                            <td class="fw-bold text-danger text-end"><fmt:formatNumber value="${gasto.monto}" type="currency" currencySymbol="-$" /></td>
                                            <td class="text-center">
                                                ...
                                                <form action="${pageContext.request.contextPath}/gastos" method="post" onsubmit="return confirm('¿Seguro que quieres eliminar este gasto?');" style="display:inline;">
...
<script>
    ...
    <c:if test="${not empty distribucionJSON and distribucionJSON ne '[]'}">
        ...
        new Chart(document.getElementById('distribucionGastosChart'), {
            type: 'doughnut',
            data: {
                labels: categorias,
                datasets: [{
